# ADR-0016 — Internacionalização: conteúdo agora, interface depois
...
**Opção 2.** A internacionalização de **conteúdo** está entregue e testada; a
tradução da **interface** fica registrada como incremento, a ser feito pelo
caminho da opção 3 (dicionário tipado, sem roteamento por locale).
...
backend. Quando a interface for traduzida, a escolha da biblioteca é revista com
o requisito real em mãos.
...
- A casca da interface está só em PT-BR; um avaliador internacional vê os
  rótulos em português (o conteúdo, não).
- A tradução futura exigirá tocar os componentes e os testes — trabalho adiado,
  não eliminado.
